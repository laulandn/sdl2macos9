...
fprintf(stderr,"BuildOffscreen...\n"); fflush(stderr);
fprintf(stderr,"wPixDepth is %d\n",wPixDepth); fflush(stderr);
...
fprintf(stderr,"fMustRebuild is %d\n",(int)fMustRebuild); fflush(stderr);
	if (fMustRebuild) // must rebuild
	{	
		if (NULL != *ppGWorld) // if we have an allocated GWorld
		{
fprintf(stderr,"already have gworld"); fflush(stderr);
...
fprintf(stderr,"No vram...\n"); fflush(stderr);
				SysBeep (30);
				*location = kInAGP;
			case kInAGP:
fprintf(stderr,"asked for agp\n"); fflush(stderr);
...
fprintf(stderr,"No agp...\n"); fflush(stderr);
				SysBeep (30);
				*location = kInSystem;
			case kInSystem:
			default:
fprintf(stderr,"asked for sys ram\n"); fflush(stderr);
...
fprintf(stderr,"we rebuilt\n"); fflush(stderr);
		return true; // we rebuilt our GWorld
	}
fprintf(stderr,"all was well\n"); fflush(stderr);
...
fprintf(stderr,"AllocBuffer...\n"); fflush(stderr);
	...
	{ 
fprintf(stderr,"not fwLocOffscreen\n"); fflush(stderr);
...
fprintf(stderr,"macos 9?\n"); fflush(stderr);
		gNewNewGWorld = true;
        }
	else
	{
fprintf(stderr,"not macos 9?\n"); fflush(stderr);
		// now it is tricky
	...
	if (!gNewNewGWorld)
	{
fprintf(stderr,"not gNewNewGWorld\n"); fflush(stderr);
		hMenu = GetMenuHandle (kMenuFile);
